...
run_dir = get_absolute_path(f'run_1')
mkdir(run_dir)
print(run_dir)
...
step_dir = os.path.join(current_dir, 'step_34')

# 设置为 ParFlow 实际运行目录
run_dir = os.path.join(current_dir, 'run_1')
os.makedirs(run_dir, exist_ok=True)

# === 复制并重命名pfb文件 ===
special_files = {'mannings2000_new.pfb': 'mannings.pfb',
                 'slopex.pfb': 'slopex.pfb',
                 'slopey.pfb': 'slopey.pfb',
                 'IndicatorFile.pfb': 'IndicatorFile.pfb',
                 'DTB.pfb': 'DTB.pfb', 
                 'mao.out.porosity.pfb': 'mao.out.porosity.pfb',
                 'mao.out.specific_storage.pfb': 'mao.out.specific_storage.pfb'
}
...
# === 复制并重命名pfb文件 ===
special_files = {
    'mao.out.press.00365.pfb': 'press.init.pfb',
    'mao.out.press.00365.pfb.dist': 'press.init.pfb.dist'
}
for src_name, dst_name in special_files.items():
...
clm_files = {
    ...
    'drv_vegm.alluv2000.dat': 'drv_vegm.alluv.dat'
}
...
model.TimingInfo.StopTime = 8760.0
...
model.Solver.ResetSurfacePressure.ThresholdPressure = 6.0
...
model.Solver.CLM.MetFilePath = "/XYFS01/HDD_POOL/bnu_xfyang/bnu_xfyangxy_1/ljx/3_run_2019/2019_data_irrigation/pfb_2019"
