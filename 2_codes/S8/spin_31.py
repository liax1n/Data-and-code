...
run_dir = get_absolute_path(f'step_31')
mkdir(run_dir)
print(run_dir)
...
step_dir = os.path.join(current_dir, 'step_1')

# 设置为 ParFlow 实际运行目录
run_dir = os.path.join(current_dir, 'step_31')
...
# === 复制并重命名pfb文件 ===
special_files = {
    'mao.out.press.07500.pfb': 'press.init.pfb',
    'mao.out.press.07500.pfb.dist': 'press.init.pfb.dist'
}
for src_name, dst_name in special_files.items():
...
model.Solver.CLM.MetFilePath = "/XYFS01/HDD_POOL/bnu_xfyang/bnu_xfyangxy_1/ljx/2_spinup/2004_data_irrigation/pfb_2004"
