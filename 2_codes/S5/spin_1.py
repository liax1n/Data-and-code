...
run_dir = get_absolute_path(f'step_1')
mkdir(run_dir)
print(run_dir)
...
input_dir= os.path.join(parent_dir, 'pfb_files')
# === 复制并重命名pfb文件 ===
...
model.TimingInfo.StopTime = 15000000.0
model.TimingInfo.DumpInterval = 2000.0

# Growth timestep properties
model.TimeStep.Type = "Growth"
model.TimeStep.InitialStep = 1
model.TimeStep.GrowthFactor = 1.1
model.TimeStep.MaxStep = 2000
model.TimeStep.MinStep = 1.0
...
model.Patch.z_upper.BCPressure.alltime.Value = -2.1e-5
...
#model.Geom.domain.ICPressure.FileName = "mao.out.press.01350.pfb"
#8.1————求解器：outputs
...
model.Solver.WriteSiloSpecificStorage = True
model.Solver.WriteSiloMannings = True
model.Solver.WriteSiloMask = False
model.Solver.WriteSiloSlopes = True
