...
# 所有文件
all_tgrnd_files = sorted(glob.glob("run_1/mao.out.eflx_sh_tot.*.pfb"))
...
for day_index, group in enumerate(daily_groups, start=1):
    ...
    plt.colorbar(im, ax=ax, label='qflx_sh_tot', shrink=0.8)
    ax.set_title(f'Day {day_index}')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')

    # 保存 PNG 并读取帧
    png_path = os.path.join(output_dir, f'qflx_sh_tot_day_{day_index:03d}.png')
...
np.save(os.path.join(output_dir, 'qflx_sh_tot.npy'), tgrnd_stack)

gif_path = os.path.join(output_dir, 'qflx_sh_tot.gif')
